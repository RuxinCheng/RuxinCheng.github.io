...
df_simulation<-df_fill[c(4,7,8,14,16),1:3] #profile 4, 6, 7, 8 and 16
...
# Therefore, we introduce three products.
# Product4, Product6, and Product16


# calculate matket share

# First Possibility
# Competitors do not react to our strategy, which means competitor now uses profile 7.

# Scenario one
# We launch profile 4 and 16.
# We calculate the profit for each possible combination.
...
profit_no_reaction = as.data.frame(c(1:14))
profit_no_reaction$profit_shorterm = first_year_profit
colnames(profit_no_reaction)[1] = "scenario"
...
profit_no_reaction$profit_longterm = second_year_profit
write.csv(profit_no_reaction,file = 'profit_no_reaction.csv')
...
#The scenario with the highest Long-Term Profit is 146931.8, when we launch product 4, 16.
...
res_new = matrix(nrow=length(scens_new),ncol=length(df_simulation_newnew))
...
profit_reaction = as.data.frame(c(1:14))
profit_reaction$profit_shorterm = first_year_profit
colnames(profit_reaction)[1] = "scenario"
...
profit_reaction$profit_longterm = second_year_profit
write.csv(profit_reaction,file = 'profit_reaction.csv')

#First year profit = $135783.2
#Long-Term Profit = $149116.6


# We run other 7 scenarios with same calculation. 
#The scenario with the highest Long-Term Profit is 149116.6, when we launch product 4, 14.
